...
class env_DLEDMD:
    def __init__(self, rainData):
        ...
        self.data=np.concatenate((dt_s0,dt_a0,dt_r0),axis=1)
        ...
        for it in range(3):
            for jt in [0,2]:   
                ...
                self.data=np.concatenate((self.data,tem),axis=0)
        ...
        dt_r0=df_r0.values[:,1:]
        self.data_test1=[]
        self.data_test1.append(np.concatenate((dt_s0,dt_a0,dt_r0),axis=1))
        ...
        for it in [0,2,3,4,5,6,7,8,9]:
            for jt in [1]:   
                ...
                dt_r0=df_r0.values[:,1:]
                tem=np.concatenate((dt_s0,dt_a0,dt_r0),axis=1)
                self.data_test1.append(tem)
                
        X_train=self.data[:self.data.shape[0]-2,:]
        Y_train=self.data[1:self.data.shape[0]-1,:]
        
        #KEDMD
        self.DL = MLPRegressor(hidden_layer_sizes=(13, 4),  activation='identity', alpha=0.1, batch_size='auto',
                  power_t=0.5, max_iter=50000,epsilon=1e-08)
        self.DL.fit(X_train,Y_train)
    ...
    def step(self,a,rain):
        ...
        self.iten+=1
        S=np.hstack((self.st,action,rain[self.iten])).reshape((1,-1))
        Y=self.DL.predict(S)     
        ...
        store=np.abs(Y[0][2])
        outflow=np.abs(Y[0][3])
        rain_sum=np.sum(rain[:self.iten])
...
if __name__=='__main__':
    ...
    rainData1=np.loadtxt('./sim/testRainFile.txt',delimiter=',')/6#读取测试降雨数据
