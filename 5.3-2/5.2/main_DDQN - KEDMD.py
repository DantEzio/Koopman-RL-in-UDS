...
if __name__=='__main__':
    ...
    rainData=rainData1=np.loadtxt('./sim/testRainFile.txt',delimiter=',')#读取测试降雨数据
    #rainData2=np.loadtxt('./sim/real_rain_data.txt',delimiter=' ')*10#读取真实降雨数据
    #rainData=np.vstack((rainData1[:,:120],rainData2[:4,:]))#共8场降雨
    
    envK=env_KEDMD.env_KEDMD(rainData)
    K_DQN = DDQN.DDQN(step=step,batch_size=batch_size,num_rain=rain_num,env=envK,t='dqn_KEDMD')
    t1=time.time()
    K_DQN.train()
    t2=time.time()
    print(t2-t1)
    ...
    K_DQN = DDQN.DDQN(step=step,batch_size=batch_size,num_rain=rain_num,env=envS,t='dqn_KEDMD')
