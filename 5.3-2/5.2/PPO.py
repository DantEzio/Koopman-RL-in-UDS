...
class PPO:
    def __init__(self, env, ep, batch, num_rain, t='ppo2'):
        self.t = t
        self.ep = ep
        ...
        #self.testRainData=np.vstack((rainData1[:,:120],rainData2[:4,:]))#共8场降雨
    ...
    def _build_critic(self):
        ...
        with tf.compat.v1.variable_scope('critic',reuse=tf.compat.v1.AUTO_REUSE):
            x = tf.layers.dense(self.states, 100, tf.nn.relu,kernel_initializer=tf.zeros_initializer(),
                                bias_initializer=tf.zeros_initializer())

            self.v = tf.layers.dense(x, 1,kernel_initializer=tf.zeros_initializer(), 
                                     bias_initializer=tf.zeros_initializer())
    ...
    def build_model(self):
        ...
        """
        # inputs
        self.states = tf.compat.v1.placeholder(tf.float32, [None, 4], 'states')
...
#               + tf.reduce_sum(tf.log(std), axis=-1)
    ...
    def test(self,test_num,repeat):
        ...
        for i in test_num:#range(test_num):
            print('test'+str(i))
            ...
            for re in range(repeat):
                ...
                while True:
                    ...
                    #uncertainty
                    X=np.random.uniform(0.95,1.05,size=next_observation.shape)
                    observation=next_observation*X
                ...
                sout='./'+self.t+'_test_result/'+str(i)+'_'+str(re)+'.rpt'
                sin=self.env.staf+'.rpt'
                self.env.copy_result(sout,sin)
                sout='./'+self.t+'_test_result/'+str(i)+'_'+str(re)+'.inp'
