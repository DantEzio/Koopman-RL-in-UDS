...
for name in names:
    for sur in surr:
        file_lists.append(['./final-noAFI-drain/'+name+'_'+sur+'_test_result/'+str(i) for i in range(8)])
        sheet_names.append(name+'_'+sur)

        
file_lists.append(['./final-noAFI-drain/HC/HC'+str(i) for i in range(8)])
sheet_names.append('hc')
print(file_lists)
        
for t in range(len(file_lists)):
    
    floods=[]
    
    file_names=file_lists[t]
    ...
    floods_results[sheet_names[t]]=floods
    
    df = pd.DataFrame(np.array(floods).T)
    df.to_excel(writer, index=False, encoding='utf-8',sheet_name=sheet_names[t])
writer.save()
