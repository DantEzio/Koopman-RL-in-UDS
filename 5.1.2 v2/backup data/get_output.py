...
def get_node_data(node_data,node_name):
    '''
    所有节点的水位、流量、COD
    '''
    #针对每个节点构建字典
    ...
    writer = pd.ExcelWriter('node_data.xlsx')
    datapd.to_excel(writer)
    writer.save()
    
def get_link_data(link_data,link_name):
    '''
    所有节点的水位、流量、COD
    '''
    #针对每个节点构建字典
    ...
    #将上述字典合并为矩阵，保存为excel
    datapd=pd.DataFrame(data)
    writer = pd.ExcelWriter('link_data.xlsx')
    datapd.to_excel(writer)
    writer.save()
    
def get_lateralin_data(node_data,node_name):
    '''
    所有节点的水位、流量、COD
    '''
    #针对每个节点构建字典
    ...
    #将上述字典合并为矩阵，保存为excel
    datapd=pd.DataFrame(data)
    writer = pd.ExcelWriter('inflow_data.xlsx')
    datapd.to_excel(writer)
    writer.save()
...
if __name__=='__main__':
    ...
    print(len(node_name))
    #get_node_data(node_data,node_name)
    get_link_data(link_data,link_name)
    get_lateralin_data(node_data,node_name)
