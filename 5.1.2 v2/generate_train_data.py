# -*- coding: utf-8 -*-
"""
Created on Mon Oct 26 21:13:38 2020
...
class training_data_gen:
    def __init__(self,ori_filename,raindata_file):
        ...
        for j in ['09','10']:
            for i in range(10):
                self.rain_time.append(j+':0'+str(i))
            for i in range(10,60):
                self.rain_time.append(j+':'+str(i))
        self.rain_time.append('11:00')
        ...
        #raindata_file='./source data/rainfall data.txt'
        self.Rains=np.loadtxt(raindata_file)
        self.num_set=1#一场降雨对应的随机泵操控数据个数
        self.num_M=self.Rains.shape[0]#降雨场数
    ...
    def one_data(self,rain,pump_actions,rainlog,i):#针对一场降雨和一组泵的运行生成一组数据
        #先改降雨，再改泵的顺序不能变
        SO.change_rain(rain,self.rain_time,self.ori_filename,self.rain_filename)
        ...
        #保存模拟结果
        savefile='./save_data/rain_P/outfile/rain'+str(rainlog)+'_set'+str(i)
    ...
    def one_setdata(self,rain,rainlog):#针对一场降雨，随机生成一组泵的运行数据，调用one_data生成数据
        ...
        for i in range(self.num_set):
            ...
            for time in range(len(self.date_time)):
                ...
                if i ==0:
                    actions.append(np.ones(len(self.pump_list)))
                else:
                    actions.append(np.random.randint(0,2,len(self.pump_list)))
            
            filename='./save_data/rain_P/outfile/rain'+str(rainlog)+'_set'+str(i)
            savename='./save_data/rain_P/excelfile/rain'+str(rainlog)+'_set'+str(i)
            n_action=np.array(actions)[1:,:]
            print(n_action.shape)
    ...
    def Get_data(self):
        print('rainfall data:', self.Rains.shape)
        #选其中self.num_M场降雨
        for rainlog in range(self.num_M):
            self.one_setdata(self.Rains[rainlog],rainlog)
    ...
    def data_transfer(self):
        ...
        for rainlog in range(self.num_M):
            for i in range(self.num_set):
                ...
                #go.get_node_data(node_data,node_name,savename)
                #go.get_link_data(link_data,link_name,savename)
                #go.get_lateralin_data(node_data,node_name,savename)
                #go.get_sub_data(sub_data,sub_name,savename)
                
                #补充每个时刻的flooding值，10min一个数据
                ...
                for i in range(1,len(self.date_time)):
                    if i >5 and i <= 18:
                        rain.append(self.Rains[rainlog][k])
                        k=k+10
                    else:
                        rain.append(0.0)
                
                datapd=pd.DataFrame(rain)
                writer = pd.ExcelWriter(savename+'_rain_data.xlsx')
                datapd.to_excel(writer)
                writer.save()
