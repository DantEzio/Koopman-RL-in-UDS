...
#        self.rain=change_rain.gen_rain(self.date_t[-1],A,C,P,b,n,R,self.deltt)
        ...
        state=np.array([outflow/(0.001+total_in),flooding/(0.001+total_in),store/(0.001+total_in),rain_sum])
...
#        _,flooding_compare,_,_,_,_=get_rpt.get_rpt('compare_tem_HC.rpt')
    ...
    def step_AFI(self,a):
        ...
        self.orftem_AFI='./sim/AFI/orf_tem'
        self.staf_AFI='./sim/AFI/staf'
        
        change_rain.copy_result(self.orftem_AFI+'.inp',self.orf_rain+'.inp')        
        self.iten_AFI=self.iten+1        
        tem_etime=self.date_time[self.iten_AFI]
        set_datetime.set_date(self.sdate,self.edate,self.stime,tem_etime,self.orftem_AFI+'.inp')
        
        #self.simulation(self.staf_AFI+'.inp')
        ...
        self.action_seq_AFI=self.action_seq
        self.action_seq_AFI.append(action)
        #print(self.action_seq_AFI)
        
        set_pump.set_pump(self.action_seq_AFI,self.date_time[1:self.iten_AFI],pumps,self.orftem_AFI+'.inp')
        tem_etime=self.date_time[self.iten_AFI]
        set_datetime.set_date(self.sdate,self.edate,self.stime,tem_etime,self.orftem_AFI+'.inp')
        change_rain.copy_result(self.staf_AFI+'.inp',self.orftem_AFI+'.inp')
        ...
        #从out和rpt文件读取sate值
        #如果iten==最终的时间步，模拟停止
        total_in,flooding,store,outflow,upflow,downflow=get_rpt.get_rpt(self.staf_AFI+'.rpt')
        return flooding
