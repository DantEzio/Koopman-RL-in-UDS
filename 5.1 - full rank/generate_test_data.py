...
class training_data_gen:
    def __init__(self,ori_filename):
        ...
        #分别只用前四场降雨
        rainData1=np.loadtxt('./source data/testRainFile.txt',delimiter=',')/6#读取测试降雨数据
        rainData2=np.loadtxt('./source data/real_rain_data.txt',delimiter=' ')*10#读取真实降雨数据
        self.rainData=np.vstack((rainData1[:,:120],rainData2[:4,:]))#共8场降雨
        #print(self.rainData.shape)
        
        self.num_set=10#一场降雨对应的随机泵操控数据个数
        self.num_M=self.rainData.shape[0]-1#降雨场数
    ...
    def data_transfer(self):
        ...
        Rains=self.rainData
        for rainlog in [7]:#range(self.num_M):
            for i in range(self.num_set):
                ...
                n_data=go.get_node_data(node_data,node_name,savename)
                s_data=go.get_sub_data(sub_data,sub_name,savename)
                #从上述1min一个选出10min一个的
                n1,n2=[],[]
                for i in range(len(self.date_time)-1):
                    n1.append(n_data[i*10])
                    n2.append(s_data[i*10])
                
                datapd=pd.DataFrame(n1)
                writer = pd.ExcelWriter(savename+'_node_data.xlsx')
                datapd.to_excel(writer)
                writer.save()
                
                datapd=pd.DataFrame(n1)
                writer = pd.ExcelWriter(savename+'_sub_data.xlsx')
                datapd.to_excel(writer)
                writer.save()
