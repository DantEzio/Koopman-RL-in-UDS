...
class kernel_edmd():
    ...
    def __init__(self, input_dim,feature_mapping1,feature_mapping2):
        '''
        ...
        self.feature_mapping_in=feature_mapping1
        self.feature_mapping_out=feature_mapping2
    ...
    def whiten(self, X, selected_feature, eps=0, return_whitened_data=False):
        #whitening transformation: (Chi-self.whiten_mean).dot(self.whiten_T.T)
        Chi = selected_feature(X)
    ...
    def whitened_feature_mapping(self, X, selected_feature):
        return np.hstack([np.ones([X.shape[0], 1]), (selected_feature(X)-self.whiten_mean).dot(self.whiten_T.T)])
    
    def train(self, X, Y, eps=0.):
        N = X.shape[0]
        self.X_train = X.copy()
        self.Y_train = Y.copy()        
        Chi_0 = self.whiten(X,self.feature_mapping_in, eps=eps, return_whitened_data=True)
        Chi_1 = self.whitened_feature_mapping(Y,self.feature_mapping_out)
    ...
    def prediction(self, x0, T, obs_mapping=None, nontrivial_mode_num=np.inf, return_std=False):
        ...
        m = K.shape[0]
        Chi_0 = self.whitened_feature_mapping(self.X_train,self.feature_mapping_in).dot(U)
        ...
        f0 = self.whitened_feature_mapping(x0.reshape(1, -1),self.feature_mapping_in).dot(U).T
...
class linear_mmd_dmd():
    ...
    def whitened_feature_mapping(self, X,selected_feature):
        return (selected_feature(X)-self.whiten_mean).dot(self.whiten_T.T)
    
    def calculate_singular_function(self, X,selected_feature):
        '''
        Left singular function from (N * input_dim) array to (N * ...) array
        '''
        return self.whitened_feature_mapping(X,selected_feature).dot(self.U)
    ...
    def prediction(self, x0, T, obs_mapping=None, nontrivial_mode_num=np.inf, return_std=False):
        ...
        """
        N = self.Y_train.shape[0]
        m = min(nontrivial_mode_num,self.s.shape[0])
        if obs_mapping is None:
            O = self.Y_train
        else:
            O = obs_mapping(self.Y_train)
        ...
        f0 = np.empty(m+1)
        f0[0] = 1
        f0[1:] = self.calculate_singular_function(x0.reshape(1, -1),self.feature_mapping_in)[0, :m]
...
def test2():
    ...
    #try 不对称输入输出
    X_train, Y_train=data[:359,:],data[1:360,9:]
    X_test, Y_test=data[:359,:],data[1:360,9:]
    print(Y_train.shape)
    
    feature_dim = 1000
    feature_Win = np.random.rand(X_train.shape[1], feature_dim) * 2 -1
    feature_bin = np.random.rand(feature_dim)
    def feature_mapping_in(X):
        return np.exp(-(X.dot(feature_Win) + feature_bin) ** 2 / 2)
    
    feature_Wout = np.random.rand(Y_train.shape[1], feature_dim) * 2 -1
    feature_bout = np.random.rand(feature_dim)
    def feature_mapping_out(X):
        return np.exp(-(X.dot(feature_Wout) + feature_bout) ** 2 / 2)
    
    model_mmd_dmd = linear_mmd_dmd(input_dim=X_train.shape[1],feature_mapping1=feature_mapping_in,feature_mapping2=feature_mapping_out)
    model_mmd_dmd.train(X_train, Y_train)
    
    model_kernel_edmd = kernel_edmd(input_dim=X_train.shape[1],feature_mapping1=feature_mapping_in,feature_mapping2=feature_mapping_out)
    model_kernel_edmd.train(X_train, Y_train)
    
    Y_out1=model_mmd_dmd.prediction(X_test[0,:],Y_test.shape[0])
    Y_out2,km=model_kernel_edmd.prediction(X_test[0,:],Y_test.shape[0])
