...
class training_data_gen:
    def __init__(self,ori_filename):
        ...
        self.num_set=3#一场降雨对应的随机泵操控数据个数
        self.num_M=10#降雨场数
        self.Rains=np.loadtxt('./source data/rainfall data.txt')*2
    ...
    def one_data(self,rain,pump_actions,rainlog,i):#针对一场降雨和一组泵的运行生成一组数据
        ...
        #保存模拟结果
        savefile='./save_data/outfile/rain'+str(rainlog)+'_set'+str(i)
    ...
    def one_setdata(self,rain,rainlog):#针对一场降雨，随机生成一组泵的运行数据，调用one_data生成数据
        ...
        for i in range(self.num_set):
            ...
            filename='./save_data/outfile/rain'+str(rainlog)+'_set'+str(i)
            savename='./save_data/excelfile/rain'+str(rainlog)+'_set'+str(i)
