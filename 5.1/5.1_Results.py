...
if __name__=='__main__':
    ...
    X_test, Y_test=data[:49,:],data[1:50,:]
    ...
    print(np.sum(np.sum(Y_out1[:,9:]-Y_test[:,9:]))/(data.shape[0]*data.shape[1]))
    print(np.sum(np.sum(Y_out2[:,9:]-Y_test[:,9:]))/(data.shape[0]*data.shape[1]))
    print(np.sum(np.sum(Y_out3-Y_test[:,9:]))/(data.shape[0]*data.shape[1]))
    ...
    for i in range(1):
        ...
        plt.plot(Y_out3[j:j+18,i],label='Linear')
        plt.plot(Y_test[j:j+18,i],label='truth')
        
        plt.legend()
        
    plt.title('Total flooding')
